/**
 * Entity extraction and tracking for news articles
 * Extracts people, organizations, and locations from news text
 */

export type EntityType = 'person' | 'organization' | 'location' | 'event' | 'weapon' | 'military_unit';
...
// ── Aggregate entities from multiple articles ────────────────────────────────
export function aggregateEntities(articles: Array<{ title: string; summary?: string }>): EntityExtractionResult {
...
export function filterEntitiesByType(entities: Entity[], type: EntityType): Entity[] {
  return entities.filter(e => e.type === type);
}

// ── Get trending entities (top N by mention count) ─────────────────────────────
export function getTrendingEntities(entities: Entity[], limit: number = 10): Entity[] {
