...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const sigint: SigintItem[] = items
      ...
      .slice(0, 6)
      .map(i => ({ title: i.title, sourceName: i.sourceName ?? '—', region: i.region ?? 'global',
                   pubDate: i.pubDate ?? new Date().toISOString(), link: i.link ?? '#' }));
...
router.get('/', (_req: Request, res: Response) => {
  return res.status(405).json({ error: 'Send a POST request with { items: FeedItem[] }' });
});
